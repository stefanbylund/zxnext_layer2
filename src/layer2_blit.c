                 ...
                 off_screen_buffer_t *off_screen_buffer)
{
    ...
    if (y < 64)
    {
        ...
        if (y + height - 1 < 64)
        {
            ...
            layer2_blit_section(x, y, source, width, height, off_screen_buffer);
        }
        else if (y + height - 1 < 128)
        {
            // top
            switch_top_screen_section(off_screen_buffer);
            layer2_blit_section(x, y, source, width, 64 - y, off_screen_buffer);

            // middle
            switch_middle_screen_section(off_screen_buffer);
            layer2_blit_section(x, 0, source + width * (64 - y), width, height - (64 - y), off_screen_buffer);
        }
        else
        {
            // top
            ...
            // middle
            switch_middle_screen_section(off_screen_buffer);
            layer2_blit_section(x, 0, source + width * (64 - y), width, 64, off_screen_buffer);

            // bottom
            switch_bottom_screen_section(off_screen_buffer);
            layer2_blit_section(x, 0, source + width * (128 - y), width, height - (128 - y), off_screen_buffer);
        }
    }
    else if (y < 128)
    {
        ...
        if (y + height - 1 < 128)
        {
            // middle
            switch_middle_screen_section(off_screen_buffer);
            layer2_blit_section(x, y - 64, source, width, height, off_screen_buffer);
        }
        else
        {
            // middle
            switch_middle_screen_section(off_screen_buffer);
            layer2_blit_section(x, y - 64, source, width, 128 - y, off_screen_buffer);
    ...
    else
    {
        // bottom
        switch_bottom_screen_section(off_screen_buffer);
        layer2_blit_section(x, y - 128, source, width, height, off_screen_buffer);
    }
